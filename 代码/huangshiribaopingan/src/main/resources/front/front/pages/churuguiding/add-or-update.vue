		...
		<form class="app-update-pv">
            <view
                ...
                <input   disabled
                         ...
                         :disabled="ro.churuguidingName" type="text" v-model="ruleForm.churuguidingName" placeholder="标题"></input>
            </view>
                <view
                        ...
                        class="cu-form-group" @tap="churuguidingPhotoTap">
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"#A27DE3","textAlign":"left"}'
                          class="title">图片</view>
                    <view class="right-input" style="padding:0;textAlign:left">
                        <image
                                ...
                                class="avator" v-if="ruleForm.churuguidingPhoto" :src="ruleForm.churuguidingPhoto" mode="aspectFill">
                        </image>
            ...
            <view
                ...
                <view :style='{"width":"160rpx","fontSize":"28rpx","color":"#A27DE3","textAlign":"left"}'
                      class="title">内容</view>
                <textarea :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"#A27DE3","borderRadius":"0px","color":"#A27DE3","textAlign":"left","borderWidth":"0 0 6rpx","fontSize":"24rpx","borderStyle":"solid","height":"68rpx","height":"200rpx"}'
                          :disabled="ro.churuguidingContent" v-model="ruleForm.churuguidingContent" placeholder="内容"/>
            </view>
	...
	export default {
		data() {
			return {
				cross: '',
                ro:{
					churuguidingName: false,
					churuguidingPhoto: false,
					insertTime: false,
					churuguidingContent: false,
					createTime: false,
				},
				ruleForm: {
					churuguidingName: '',
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				let res = await this.$api.info(`churuguiding`, this.ruleForm.id);
				this.ruleForm = res.data;
			}
			if(options.churuguidingId){
                this.ruleForm.churuguidingId = options.churuguidingId;
			}
			// 跨表
			// this.styleChange()
		},
		methods: {
			...
			churuguidingPhotoTap() {
				let _this = this;
				this.$api.upload(function(res) {
					_this.ruleForm.churuguidingPhoto = _this.$base.url + 'upload/' + res.file;
					_this.$forceUpdate();
					_this.$nextTick(() => {
						// _this.styleChange()
					})
			...
			async onSubmitTap() {
				if ((!this.ruleForm.churuguidingName)) {
					this.$utils.msg(`标题不能为空`);
					return
				}
				if ((!this.ruleForm.churuguidingPhoto)) {
					this.$utils.msg(`图片不能为空`);
					return
				}
				if (this.ruleForm.id) {
					await this.$api.update(`churuguiding`, this.ruleForm);
				} else {
					await this.$api.save(`churuguiding`, this.ruleForm);
				}
